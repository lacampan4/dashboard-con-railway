...
(function () {
  ...
  // Indicador de progreso separado del botón. Así el usuario puede ver
  // claramente la barra y el contador de registros sin cambiar el diseño
  // del botón "Actualizar desde SAP".
  function ensureProgressIndicator(button) {
    if (!button || !button.parentNode) return null;
    let wrap = document.getElementById('sapProgressWrap');
    if (wrap) return wrap;

    wrap = document.createElement('span');
    wrap.id = 'sapProgressWrap';
    wrap.style.display = 'none';
    wrap.style.alignItems = 'center';
    wrap.style.gap = '8px';
    wrap.style.minWidth = '245px';
    wrap.style.height = '40px';
    wrap.style.boxSizing = 'border-box';
    wrap.style.padding = '5px 9px';
    wrap.style.border = '1px solid var(--line2, #d2d6db)';
    wrap.style.borderRadius = '10px';
    wrap.style.background = 'var(--panel, #fff)';
    wrap.style.verticalAlign = 'middle';

    const track = document.createElement('span');
    track.id = 'sapProgressTrack';
    track.style.position = 'relative';
    track.style.display = 'block';
    track.style.flex = '1 1 auto';
    track.style.minWidth = '105px';
    track.style.height = '9px';
    track.style.overflow = 'hidden';
    track.style.borderRadius = '999px';
    track.style.background = 'var(--bg2, #edf0f3)';

    const fill = document.createElement('span');
    fill.id = 'sapProgressFill';
    ...
    fill.style.background = 'linear-gradient(90deg, #e10600, #ef6b65)';
    fill.style.transition = 'width .25s ease';
    track.appendChild(fill);

    const label = document.createElement('span');
    label.id = 'sapProgressLabel';
    label.style.fontFamily = 'IBM Plex Mono, monospace';
    label.style.fontSize = '11px';
    label.style.fontWeight = '600';
    label.style.whiteSpace = 'nowrap';
    label.style.color = 'var(--txt2, #445)';
    label.textContent = '0 / 0';

    const pctLabel = document.createElement('span');
    pctLabel.id = 'sapProgressPct';
    pctLabel.style.fontFamily = 'Oswald, sans-serif';
    pctLabel.style.fontSize = '12px';
    pctLabel.style.fontWeight = '600';
    pctLabel.style.whiteSpace = 'nowrap';
    pctLabel.style.color = 'var(--txt, #14161a)';
  ...
  function hideProgressIndicator(button) {
    const wrap = document.getElementById('sapProgressWrap');
    if (!wrap) return;
    wrap.style.display = 'none';
  ...
  function setProgressIndicator(button, percent, processed, total, active = true, indeterminate = false) {
    const wrap = ensureProgressIndicator(button);
    if (!wrap) return;
    const fill = document.getElementById('sapProgressFill');
    const label = document.getElementById('sapProgressLabel');
    const pctLabel = document.getElementById('sapProgressPct');
    ...
    if (indeterminate) {
      fill.style.width = '35%';
      fill.style.animation = 'lcSapProgressIndeterminate 1.1s ease-in-out infinite';
      label.textContent = `${Number(processed || 0).toLocaleString('es-CO')} registros`;
      pctLabel.textContent = '…';
    } else {
      fill.style.animation = 'none';
      fill.style.width = pct + '%';
      label.textContent = `${Number(processed || 0).toLocaleString('es-CO')} / ${Number(total || 0).toLocaleString('es-CO')}`;
  ...
  function setButton(button, text, disabled) {
    ...
    if (!disabled && button.dataset.sapProgressActive === '1') {
      button.style.background = button.dataset.sapOriginalBackground || '';
      button.style.backgroundSize = '';
      button.dataset.sapProgressActive = '0';
      hideProgressIndicator(button);
    }
  }
  ...
  function setButtonProgress(button, text, percent, processed, total) {
    ...
    // El botón conserva su aspecto normal; el progreso se muestra en el
    // indicador separado que aparece inmediatamente a su lado.
    button.style.background = button.dataset.sapOriginalBackground || '';
    setProgressIndicator(button, pct, processed, total, true, Number(total) <= 0);
  }
