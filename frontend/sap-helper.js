...
(function () {
  ...
  function setButtonProgress(button, text, percent) {
    if (!button) return;
    button.dataset.sapOriginalText ||= button.textContent.trim();
    if (button.dataset.sapProgressActive !== '1') {
      button.dataset.sapOriginalBackground = button.style.background || '';
    }
    const pct = Math.max(0, Math.min(100, Number(percent) || 0));
    button.textContent = text;
    button.disabled = true;
    button.dataset.sapProgressActive = '1';
    // La barra vive dentro del propio botón: no agrega otro control ni mueve el layout.
    button.style.background = `linear-gradient(to right, rgba(255,255,255,.30) ${pct}%, rgba(255,255,255,.08) ${pct}%)`;
  }
    ...
    while (true) {
      ...
      if (status.ejecutando) {
        ...
        if (total > 0) {
          ...
          setButtonProgress(button, `SAP: ${processed.toLocaleString('es-CO')}/${total.toLocaleString('es-CO')} (${pct.toFixed(0)}%)`, pct);
        } else {
          // Mientras SAP no entregue __count mostramos páginas procesadas.
          setButtonProgress(button, `SAP: procesando ${processed.toLocaleString('es-CO')} registros · pág. ${page}`, 0);
        }
        await new Promise(resolve => setTimeout(resolve, POLL_INTERVAL_MS));
        continue;
      }
