/* La Campana - integración del botón "Actualizar desde SAP".
   No usa Google Drive. La sincronización se ejecuta en Railway y el botón
...
(function () {
  ...
  // Antes el valor por defecto era 'https://dashboard-con-railway-production.up.railway.app/api' (una
  // URL vieja de pruebas). El backend real de este proyecto es Railway — y ninguna
  ...
  const API_BASE = (window.LC_API_BASE || 'https://dashboard-con-railway-production.up.railway.app/api').replace(/\/$/, '');
  ...
  // La sincronización SAP ya está fragmentada por meses en Railway.
  ...
  // /sync-sap inicia el trabajo en segundo plano en Railway, por lo que el
  ...
  const MAX_CONSECUTIVE_POLL_FAILURES = 60; // hasta ~2 min de desconexión temporal
  const MAX_MS_SIN_PROGRESO = 60 * 60 * 1000; // 60 min sin avance real => posible atasco
    ...
    while (true) {
      ...
      try {
        ...
        if (consecutiveFailures >= MAX_CONSECUTIVE_POLL_FAILURES) {
          ...
          // Railway puede seguir trabajando igual.
          throw new Error(
      ...
      if (info.ejecutando) {
        ...
        if (progressKey !== lastProgressKey) {
          ...
          throw new Error(
            ...
            'Revisa /sync-status en Railway para confirmar si sigue corriendo.'
          );
        }

        if (total > 0) {
          const pct = Math.min(100, (processed / total) * 100);
          setButtonProgress(button, `SAP: ${processed.toLocaleString('es-CO')}/${total.toLocaleString('es-CO')} (${pct.toFixed(0)}%)`, pct, processed, total);
        } else {
  ...
  function enhanceExcelUploadUI() {
    if (document.getElementById('lcExcelUploadStyles')) return;

    const style = document.createElement('style');
    style.id = 'lcExcelUploadStyles';
    style.textContent = `
      .lc-xls-box{max-width:820px!important;width:min(820px,94vw)!important;border-radius:18px!important;box-shadow:0 24px 70px rgba(15,23,42,.20)!important;border:1px solid rgba(15,23,42,.08)!important}
      .lc-xls-head{background:linear-gradient(180deg,#fff,#fafbfc);padding:20px 22px!important}
      .lc-xls-eyebrow{font-family:'Oswald',Arial,sans-serif;font-size:10px;letter-spacing:1.2px;text-transform:uppercase;color:#8a94a3;margin-bottom:4px}
      .lc-xls-title{font-family:'Oswald',Arial,sans-serif;font-size:20px!important;letter-spacing:.2px}
      .lc-xls-body{padding:22px!important}
      .lc-xls-intro{display:flex;gap:12px;align-items:flex-start;padding:13px 15px;margin:0 0 15px;background:#f6f8fa;border:1px solid #e4e8ed;border-radius:12px;color:#46515d;line-height:1.5;font-size:13px}
      .lc-xls-intro-icon{width:30px;height:30px;flex:0 0 30px;border-radius:9px;background:#fff;border:1px solid #dfe4e9;display:flex;align-items:center;justify-content:center;color:#c90000;font-weight:800;font-size:15px}
      .lc-xls-formats{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 15px}
      .lc-xls-format{display:inline-flex;align-items:center;gap:6px;padding:7px 10px;border-radius:9px;border:1px solid #e0e5ea;background:#fff;color:#46515d;font-size:11px;font-weight:600}
      .lc-xls-format strong{font-family:'IBM Plex Mono',monospace;font-size:10px;letter-spacing:.3px}
      .lc-xls-format.recommended{border-color:#b9dfce;background:#f3faf7;color:#176b4a}
      .lc-xls-format .dot{width:6px;height:6px;border-radius:50%;background:#aab2bb}
      .lc-xls-format.recommended .dot{background:#1f8a5b}
      .lc-xls-drop{position:relative!important;min-height:145px!important;border:1.5px dashed #c8d0d8!important;border-radius:14px!important;background:linear-gradient(180deg,#fbfcfd,#f7f9fb)!important;transition:border-color .18s,background .18s,transform .18s,box-shadow .18s!important}
      .lc-xls-drop:hover,.lc-xls-drop.over{border-color:#c90000!important;background:#fff8f8!important;box-shadow:0 8px 24px rgba(201,0,0,.07)!important;transform:translateY(-1px)}
      .lc-xls-drop .lc-xls-drop-icon{width:42px;height:42px;border-radius:12px;background:#fff;border:1px solid #e1e5e9;display:flex;align-items:center;justify-content:center;color:#c90000;font-size:20px;box-shadow:0 3px 10px rgba(0,0,0,.05)}
      .lc-xls-drop .lc-xls-drop-main{font-size:13px;color:#27313a}
      .lc-xls-drop .lc-xls-drop-sub{font-size:11px;color:#89929c;margin-top:-4px}
      .lc-xls-drop .lc-xls-drop-sub b{color:#4c5660}
      .lc-xls-help{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin:14px 0 0}
      .lc-xls-help-card{padding:11px 12px;border:1px solid #e7eaee;border-radius:11px;background:#fff}
      .lc-xls-help-card b{display:block;font-size:11px;margin-bottom:3px;color:#27313a}
      .lc-xls-help-card span{display:block;font-size:10.5px;line-height:1.45;color:#7b858f}
      .lc-xls-result{padding:0!important;margin-top:12px!important;font-size:12px!important}
      .lc-xls-schema{margin-top:16px!important;border:1px solid #e5e9ed!important;border-radius:12px!important;overflow:hidden}
      .lc-xls-schema > b{display:block;padding:11px 13px;background:#f7f8fa}
      .lc-xls-schema table{margin:0!important}
      @media(max-width:620px){.lc-xls-help{grid-template-columns:1fr}.lc-xls-body{padding:16px!important}.lc-xls-head{padding:17px!important}}
    `;
    document.head.appendChild(style);

    const modals = [];
    const generic = document.getElementById('lcExcelModal');
    const custom = document.getElementById('xlsModal');
    if (generic) modals.push({modal:generic, inputId:'lcExcelInput', dropId:'lcExcelDrop', resultId:'lcExcelResult', titleId:'lcExcelTitle', descId:'lcExcelDesc'});
    if (custom) modals.push({modal:custom, inputId:'xlsInput', dropId:'xlsDrop', resultId:'xlsResult'});

    modals.forEach(cfg => {
      const box = cfg.modal.querySelector('.box');
      const head = cfg.modal.querySelector('.mh');
      const body = cfg.modal.querySelector('.mb');
      const drop = document.getElementById(cfg.dropId);
      if (!box || !head || !body || !drop) return;
      box.classList.add('lc-xls-box');
      head.classList.add('lc-xls-head');
      body.classList.add('lc-xls-body');

      if (!head.querySelector('.lc-xls-eyebrow')) {
        const eyebrow = document.createElement('div');
        eyebrow.className = 'lc-xls-eyebrow';
        eyebrow.textContent = 'Importación de datos';
        const h = head.querySelector('h3');
        if (h) { h.classList.add('lc-xls-title'); h.parentNode.insertBefore(eyebrow, h); }
      }

      const desc = cfg.descId ? document.getElementById(cfg.descId) : body.querySelector('p');
      if (desc) {
        desc.classList.add('lc-xls-intro');
        if (!desc.querySelector('.lc-xls-intro-icon')) {
          const icon = document.createElement('span');
          icon.className = 'lc-xls-intro-icon';
          icon.textContent = '✓';
          desc.insertBefore(icon, desc.firstChild);
        }
      }

      if (!body.querySelector('.lc-xls-formats')) {
        const formats = document.createElement('div');
        formats.className = 'lc-xls-formats';
        formats.innerHTML = `
          <span class="lc-xls-format recommended"><i class="dot"></i><strong>.XLSX</strong> recomendado</span>
          <span class="lc-xls-format"><i class="dot"></i><strong>.XLS</strong> compatible</span>
          <span class="lc-xls-format"><i class="dot"></i><strong>.CSV</strong> compatible</span>`;
        drop.parentNode.insertBefore(formats, drop);
      }

      drop.classList.add('lc-xls-drop');
      const oldIcon = drop.querySelector('div:first-child');
      if (oldIcon && !oldIcon.classList.contains('lc-xls-drop-icon')) {
        oldIcon.className = 'lc-xls-drop-icon';
        oldIcon.innerHTML = '↥';
      }
      const children = Array.from(drop.children);
      if (children[1]) children[1].className = 'lc-xls-drop-main';
      if (!drop.querySelector('.lc-xls-drop-sub')) {
        const sub = document.createElement('div');
        sub.className = 'lc-xls-drop-sub';
        sub.innerHTML = 'Formato recomendado: <b>Excel .xlsx</b> · también acepta .xls y .csv';
        drop.appendChild(sub);
      }

      if (!body.querySelector('.lc-xls-help')) {
        const help = document.createElement('div');
        help.className = 'lc-xls-help';
        help.innerHTML = `
          <div class="lc-xls-help-card"><b>✓ Para mejores resultados</b><span>Usa el archivo exportado directamente, sin convertirlo ni cambiar los encabezados.</span></div>
          <div class="lc-xls-help-card"><b>↳ Si tienes CSV</b><span>También funciona siempre que conserve la fila de encabezados y las columnas esperadas.</span></div>`;
        drop.insertAdjacentElement('afterend', help);
      }

      const result = cfg.resultId ? document.getElementById(cfg.resultId) : null;
      if (result) result.classList.add('lc-xls-result');
      const schema = body.querySelector('.schema');
      if (schema) schema.classList.add('lc-xls-schema');
    });
  }
