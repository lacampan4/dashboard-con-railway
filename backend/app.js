...
// ============================================================
// El frontend actual usa /auth/login y /auth/me. Esta implementación
// usa credenciales y secreto definidos como variables de entorno en
// Railway, sin guardar contraseñas en el código.
...
function crearTokenAuth(usuario) {
  ...
  const firma = crypto.createHmac('sha256', AUTH_SECRET).update(body).digest('base64url');
  return body + '.' + firma;
}
...
function verificarTokenAuth(token) {
  ...
  const esperada = crypto.createHmac('sha256', AUTH_SECRET).update(body).digest('base64url');
...
// SAP_CACHE_MINUTOS en Railway (0 desactiva el caché por completo).
...
// Compatibilidad con el frontend que llama a /api/* desde Vercel.
// Internamente las rutas siguen definidas como /health, /dataset, etc.
...
app.post('/auth/login', (req, res) => {
  if (!ADMIN_USERNAME || !ADMIN_PASSWORD || !AUTH_SECRET) {
    return res.status(503).json({
      ok: false,
      message: 'La autenticación no está configurada en el servidor. Configure ADMIN_USERNAME, ADMIN_PASSWORD y AUTH_SECRET.'
    });
  }
  ...
  return res.json({
    ok: true,
    token: crearTokenAuth(username),
    usuario: { username, rol: ADMIN_ROLE }
  });
});
...
app.get('/auth/me', (req, res) => {
  ...
  const payload = verificarTokenAuth(token);
  if (!payload) return res.status(401).json({ ok: false, message: 'Sesión no válida o expirada.' });
  return res.json({ ok: true, usuario: { username: payload.sub, rol: payload.rol || ADMIN_ROLE } });
});
  ...
  if (!forzar) {
    ...
    if (entry) {
      return res.status(200).json({
        ok: true,
        yaSincronizado: true,
        mensaje: `Este rango ya se consultó hace poco (${entry.totalRegistros} registros agregados${entry.filasCrudasTruncadas ? `, detalle crudo limitado a las últimas ${MAX_FILAS_CRUDAS_CACHE}` : ''}, guardados en memoria de Railway). No se volvió a consultar SAP.`,
